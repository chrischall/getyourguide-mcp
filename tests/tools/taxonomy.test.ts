...
const envelope = {
  ...
  data: {
    tours: [{ tour_id: 1, title: 'Walking tour', coordinates: { lat: 0 } }],
  },
};
...
describe('gyg_list_categories', () => {
  it('GETs /categories with pagination and language', async () => {
    ...
    setup(client);
    const result = await handlers.get('gyg_list_categories')!({
      language: 'en',
      limit: 3,
      offset: 1,
    });
...
describe('gyg_list_category_tours', () => {
  it('GETs /tours filtered by categories[] and passes the full envelope through on view:"full"', async () => {
    ...
    setup(client);
    const result = await handlers.get('gyg_list_category_tours')!({
      view: 'full',
      categoryId: 9,
      currency: 'EUR',
      limit: 2,
  ...
  it('never sends view upstream as a query param', async () => {
    const client = makeClient(envelope);
    setup(client);
    await handlers.get('gyg_list_category_tours')!({
      categoryId: 9,
      view: 'full',
    });
    expect(client.get).toHaveBeenCalledWith('/tours', {
...
describe('gyg_get_location', () => {
  it('GETs /locations/{id}', async () => {
    ...
    expect(client.get).toHaveBeenCalledWith('/locations/57', {
      cnt_language: 'fr',
    });
  });
});
...
describe('gyg_list_location_tours', () => {
  it('GETs /locations/{id}/tours and passes through on view:"full"', async () => {
    const client = makeClient(envelope);
    setup(client);
    const result = await handlers.get('gyg_list_location_tours')!({
      view: 'full',
      locationId: 57,
      limit: 1,
      offset: 0,
  ...
  it('never sends view upstream as a query param', async () => {
    const client = makeClient(envelope);
    setup(client);
    await handlers.get('gyg_list_location_tours')!({
      locationId: 57,
      view: 'full',
    });
    expect(client.get).toHaveBeenCalledWith('/locations/57/tours', {
