...
const envelope = {
  ...
  data: {
    tours: [{ tour_id: 23776, title: 'Louvre', pictures: [{ url: 'fat.jpg' }] }],
  },
};
...
describe('gyg_search_tours', () => {
  ...
  it('passes full datetimes through and sends a single-value date[] for dateFrom alone', async () => {
    ...
    await handlers.get('gyg_search_tours')!({
      view: 'full',
      dateFrom: '2026-08-01T12:30:00',
    });
    expect(client.get).toHaveBeenCalledWith(
      '/tours',
      expect.objectContaining({ 'date[]': ['2026-08-01T12:30:00'] }),
    );
  });
  ...
  it('rejects dateTo without dateFrom with an actionable error', async () => {
    ...
    setup(client);
    await expect(
      handlers.get('gyg_search_tours')!({ view: 'full', dateTo: '2026-08-05' }),
  ...
  it('never sends view upstream as a query param', async () => {
    ...
    const withoutView = (client.get as unknown as { mock: { calls: unknown[][] } }).mock
      .calls[0][1];
...
describe('gyg_get_tour', () => {
  ...
  it('GETs /tours/{id} with currency/language overrides', async () => {
    ...
    setup(client);
    const result = await handlers.get('gyg_get_tour')!({
      tourId: 23776,
      currency: 'USD',
      language: 'de',
    });
...
describe('gyg_get_tour_availability', () => {
  it('GETs /tours/{id}/availability with the hyphenated cnt-language and no defaults', async () => {
    ...
    setup(client);
    const result = await handlers.get('gyg_get_tour_availability')!({
      tourId: 23776,
      language: 'de',
    });
    expect(client.get).toHaveBeenCalledWith(
