...
function setup(
  ...
  registerHealthcheckTools(
    server,
    { get } as unknown as GYGClient,
    (k: string) => env[k],
  );
  const call = async () =>
    JSON.parse(
      (await (server as any)._registeredTools.gyg_healthcheck.handler({}, {}))
        .content[0].text,
    );
...
describe("gyg_healthcheck", () => {
  it("registers under the repo tool prefix", () => {
    expect(Object.keys((setup(FULL).server as any)._registeredTools)).toEqual([
      "gyg_healthcheck",
    ]);
  });
  ...
  it("keys only on text client.ts actually produces", () => {
    ...
    for (const [arm, text] of Object.entries(CLIENT_ERROR_TEXT)) {
      expect(
        clientSource,
        `${arm}: "${text}" no longer appears in client.ts`,
      ).toContain(text);
    }
  });
